...
> submission metrics remain zero. This package now contains both a canonical
> Bitcoin indexing tranche and an inert durable signed-transaction outbox
> protocol, but no bundled production runtime yet composes them with the
> required Ethereum lifecycle index, transactional challenge persistence,
> broadcaster, and independent reconciliation. COMPLETE_V2 activation also
...
> current result is empty; wallet-history and sequential outspend polling are
> not a canonical Bitcoin change feed and cannot certify point-in-time coverage.
> Production activation additionally requires a keyed deposit database,
> bounded new-block input matching, block-hash rollback, and a durable unmatched
> Ethereum-proof backlog.
