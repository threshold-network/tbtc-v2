...
async function createTestDatabase(
  ...
  const migrations = [
    ...
    new URL(
      "../migrations/016_p2tr_signer_boundary_late_artifact.sql",
      import.meta.url
    ),
    new URL(
      "../migrations/017_p2tr_signer_boundary_nonce_finality.sql",
      import.meta.url
    ),
    new URL(
      "../migrations/018_p2tr_signed_variant_exact_gas.sql",
      import.meta.url
    ),
  ]
...
postgresTest(
  "replays grandfathered v4 signer-boundary evidence after migration 017",
  async () => {
    const database = await createTestDatabase(
      1_024,
      CHAIN_ID,
