...
This section governs **seat/signing weight only**. The caps here bound signing concentration — the decentralization property that protects custody. They do **not** cap revenue: reward weight is uncapped and tracks delegated capital (§8). For allowlisted Active operators:

```
rawWeight  = selfBond + delegatedAssets − pendingUndelegations
weight     = min(rawWeight, selfBond × λ, maxOperatorWeight)   // SEAT weight, capped
...
**Distribution — reward weight is uncapped, seat weight is capped.** Reward allocation across operators tracks each operator's **uncapped delegated capital** (`rewardWeight = selfBond + delegatedAssets`, gated only on the operator being an eligible active signer), _not_ the capped seat weight of §5. `accPerWeight += reward × 1e18 / totalRewardWeight` per `distribute()`, checkpointed per operator on every capital or active-status change. Within a pool: `commissionBps × R` to the operator beneficiary; the rest credited to the pool's share price (self-bond participates pro-rata too, so operators earn commission + their own stake's share). Delegators `claimRewards(provider)` in TBTC.

This deliberately decouples revenue from the seat cap. The cap exists to bound **signing concentration** — the decentralization property that actually protects custody (§5, enforced at seat allocation and by `maxSeatsPerWallet`). Revenue is a distributional matter, not a custody-security one: an operator who attracts more delegated capital earns proportionally more even past the point where that capital buys additional seats, which is fair and matches the "solicit delegation to increase rewards" premise without a ceiling. Consequences, accepted by design: (1) reward share can concentrate on operators who attract the most capital, while seats/signing power cannot — the two are decoupled on purpose; (2) the self-bond leverage bound λ gates only seat weight, so an operator can run a large _rewarded_ pool on modest self-bond (seats still capped, delegators still protected by the first-loss self-bond tranche on slashing); (3) there is **no dilution cliff** — over-cap delegation raises the pool's reward slice proportionally, so it never dilutes existing delegators (the failure mode a seat-capped reward weight would have). Pending-exit capital keeps earning until finalize (§6), consistent with staying slashable — with one boundary: the "keeps earning" rule is about **delegator** undelegations, whose operator stays an eligible signer. An **operator** who drops their own effective self-bond below `minSelfBond` (including via a queued self-bond withdrawal) ceases to be a qualified signer, so their whole pool's reward weight goes to 0 immediately — the same wind-down state as deactivation/ejection, reached by another path. This is conservative and safe (never over-pays, never pays a non-signer); delegators of a winding-down operator stop accruing and drain out under the normal lifecycle gate.
...
- **Phantom-weight, capped regime (accepted residual).** The exit gate advances an exposure floor at authorization-decrease approval so exiting stake cannot leave while a wallet its weight influenced is live (implemented in `SeatAllocator`). When an operator's weight is already clamped at `selfBond × λ` or `maxOperatorWeight`, a partial undelegation does not move the synced weight, so no decrease is filed and the floor is not advanced — a delegator could then finalize while a post-request wallet its stake still slashably backs is live. This is accepted, not closed: the §5 selection-weight invariant is intact (the capped stake contributed zero marginal seats), self-bond first-loss is untouched, and flat per-seat slashing is orders of magnitude below the pool, so coverage is unaffected in practice. The strict-coupling alternative (over-lock every capped-operator exit against all their wallets) was rejected as worse for delegation liveness than the nuance it closes.
