...
- **Delegated collateral is not lifecycle-coupled on the undelegation route (accepted residual — the load-bearing consequence of equal weight).** Because a delegator undelegation no longer moves seat weight, it files no authorization decrease and the exposure floor is not advanced, so the exit gate reduces to the plain epoch gate: a delegator finalizes once the wallets that existed at request time (`epoch ≤ epochAtRequest`) drain, while the exiting shares stay slashable-and-earning until finalize. Consequence: the operator keeps signing new wallets at the same flat weight, and Model-B slashing draws on the delegated pool of _every_ wallet the operator sits in — so a delegator can finalize (withdraw) while a **post-request** wallet its capital still slashably backed is live. **The guaranteed slashing collateral for any live wallet is therefore its seats' self-bond first-loss tranche** (which _is_ lifecycle-coupled — a self-bond withdrawal crossing `minSelfBond` drives seat weight → 0, the decrease two-step, and the floor advance); delegated collateral above it is not guaranteed for post-request wallets. This is broader than the prior model's over-cap-only residual (it is now the whole delegated tranche), but economically immaterial: per-seat self-bond (`minSelfBond` 40,000 T) is far above realistic per-seat slash amounts (~500–5,000 T), so first-loss over-covers regardless. The two alternatives are rejected: gating on _all_ the operator's live wallets traps delegators indefinitely while the operator stays active; epoch-cohort-aware slashing (excluding pending shares from post-request wallets' slashable base) is exactly the accounting complexity equal weight was chosen to shed, for a benefit that self-bond first-loss already provides. Revisit only if slash amounts are ever raised toward self-bond (§9's griefing gate).
