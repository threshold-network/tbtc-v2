...
- **Allowlist curation is the decentralization control.** With uniform weight over N allowlisted operators, each gets ≈ `groupSize/N` seats in expectation, so K colluding identities need K/N ≥ the signing-threshold share — i.e. a **majority of the allowlist** — to capture a wallet, *regardless of capital*. Corrupting signing means corrupting the allowlist, which was always the real trust boundary. No `λ`, no `maxOperatorWeight` — those existed only to bound stake-weighted concentration and are removed.
- **`maxSeatsPerWallet` = 12 (retained as a guard, no longer load-bearing).** Enforced at DKG-result validation (reject any member set where one pool-ID-resolved operator exceeds the cap). Under equal weight its anti-cartel role is subsumed by uniformity — each operator's per-wallet share is already ≈ `100/N` — so it degrades to a **variance/per-node-load guard**: it bounds the multinomial tail (an operator occasionally drawing many seats) and per-seat client cost (DKG messages, signing shares, anchor-admission slots). **Liveness note:** because the cap is enforced by *rejecting* an over-cap `selectGroup` result, a too-small allowlist can still churn/brick DKG from sampling variance (at N near `groupSize/cap` ≈ 9, expected seats ≈ 11 and P(>12) is high). Mitigation under equal weight is trivial: keep the allowlist comfortably above that (target N ≥ ~15–20, expected ≤ ~5–7 seats/op, negligible over-cap), and/or relax the cap. No stake-weighted whale can force the brick — the failure is now purely a function of allowlist *size*, a DAO-controlled parameter.
...
| **`maxSeatsPerWallet`**                        | **12** (DKG validator; variance/load guard)        | Council                      | 48h                                                                                                                    |
...
**Phase 2 — open delegation.** Flip the delegation flag (delayed action); equal seat weight and the `maxSeatsPerWallet` guard live. _Exit criteria: delegations to ≥5 distinct operators; one delegator exit finalized through the full lifecycle gate; one slash borne pro-rata including a pending exit; no lifecycle stall attributable to the module. Reversion: flag off — new delegations blocked, existing positions exit normally._
...
- **Allowlist sizing is now a liveness parameter (§5).** With equal weight, the `maxSeatsPerWallet` guard can churn/brick DKG only when the allowlist is too small (sampling variance pushes an operator past the cap). No stake-weighted whale can force it — it is purely a function of N. Keep the eligible allowlist comfortably above `groupSize/cap` (target N ≥ ~15–20) and/or relax the cap. Open: whether to add an on-chain minimum-eligible-operators precondition at `requestNewWallet` for a hard guarantee.
...
- **Delegated collateral is not lifecycle-coupled on the undelegation route (accepted residual — the load-bearing consequence of equal weight).** Because a delegator undelegation no longer moves seat weight, it files no authorization decrease and the exposure floor is not advanced, so the exit gate reduces to the plain epoch gate: a delegator finalizes once the wallets that existed at request time (`epoch ≤ epochAtRequest`) drain, while the exiting shares stay slashable-and-earning until finalize. Consequence: the operator keeps signing new wallets at the same flat weight, and Model-B slashing draws on the delegated pool of *every* wallet the operator sits in — so a delegator can finalize (withdraw) while a **post-request** wallet its capital still slashably backed is live. **The guaranteed slashing collateral for any live wallet is therefore its seats' self-bond first-loss tranche** (which *is* lifecycle-coupled — a self-bond withdrawal crossing `minSelfBond` drives seat weight → 0, the decrease two-step, and the floor advance); delegated collateral above it is not guaranteed for post-request wallets. This is broader than the prior model's over-cap-only residual (it is now the whole delegated tranche), but economically immaterial: per-seat self-bond (`minSelfBond` 40,000 T) is far above realistic per-seat slash amounts (~500–5,000 T), so first-loss over-covers regardless. The two alternatives are rejected: gating on *all* the operator's live wallets traps delegators indefinitely while the operator stays active; epoch-cohort-aware slashing (excluding pending shares from post-request wallets' slashable base) is exactly the accounting complexity equal weight was chosen to shed, for a benefit that self-bond first-loss already provides. Revisit only if slash amounts are ever raised toward self-bond (§9's griefing gate).
- **Ledger reconcile is forward-only (operational mitigation).** `reconcileWalletExposure` rebuilds a desynced exposure ledger from registry-authoritative state (members verified against `membersIdsHash`), closing the premature-unlock hole from a swallowed registration hook going forward. Because epoch assignment is lazy, a recovery re-records the wallet at a fresh epoch, so an undelegation already in-flight during the swallow window (itself behind the ledger code-guard — a triple-conditional edge) is not retroactively re-gated. Mitigation: monitor the `WalletExposureLedgerCallFailed` event and reconcile well inside `undelegationDelay` (45 days). Follow-up: record recovered wallets at a floor epoch to over-lock-and-cover in-flight exits, fully closing the unsafe direction in-code.
