...
* `setRebateStaking(address rebateStaking)` forwarding to the
  Bridge via a minimal `IBridgeRebateStaking` interface, and
* The constructor:
...
* Internal:: `IBridgeRebateStaking(address(bridge)).setRebateStaking(...)`
* Bridge implementation:: `Bridge.setRebateStaking(RebateStakingProxy)`
