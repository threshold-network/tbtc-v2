...
broken or if something goes wrong, then after `refundLocktime` (180 days for
receipts generated by the tBTC SDK), they can send their funds back to
`refundPubkeyHash` themselves. This would only work if the wallet hadn't
<<sweeping,touched>> those funds yet.
