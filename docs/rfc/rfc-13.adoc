...
* *snapshots* every proof- and settlement-critical parameter (source anchor
  outpoint hash, tx max fee, redeemer script hash, target wallet, expected
  main-UTXO hash, fee-paid flag, and the three-level watchtower delay schedule
  for redemptions) — later governance changes never affect an in-flight
  generation.

*Authorization.* For reserved redemptions, the request is
authorized-for-signing only after its snapshotted watchtower delay elapses
...
* If the wallet never anchors, the authorization times out (capacity
  ...
  snapshotted refund deadline — or immediately when reveal-ahead validation
  was disabled at reveal — a stale reserved deposit with no live
  authorization can be permissionlessly marked stale, decrementing the
  pending counter.
...
dissolution-bound in behavior. For a Live custody wallet, the hard
stranding bound after the final expiry is the dissolution delay plus one
action timeout to move the wallet into MovingFunds and a second dissolution
action timeout to terminate it (plus request, notification, and settlement
inclusion latency). A still-armed moving-funds timeout may terminate the
wallet earlier, but it is not part of the hard bound: a successful main-funds
proof can clear that deadline while the residual reservation remains. A
custody wallet already in MovingFunds at the first dissolution timeout needs
only that one action timeout. For a governance intervention, the bound
additionally allows one final front-run term: detection-to-block latency +
window + term + max(delay, redemption timeout) + settlement latency.
...
objection count. At request time the Bridge snapshots the watchtower's
applicable three-level delay schedule (2h/8h/24h ladder shared with the
pooled path). Later manager delay or waiver updates apply only to new
generations, so an already-authorized spend cannot have its veto window
reopened. Permanent watchtower shutdown remains a deliberate global override
that makes every delay zero. The Bridge proof path consults the generation
snapshot (H-03), and the request path uses a reservation-scoped safety check
...
* Reserved redemption requests require the wallet to be Live or
  ...
  wallet state.
* A *terminated* wallet's active reservations can be permissionlessly marked
  `Stranded` (per position): counters release, and the owner's minted TBTC
  simply remains a fungible pooled claim — the backing shortfall is
  socialized exactly like a terminated wallet's main UTXO today. A pending
  generation cannot be stranded or have its escrow unwound: its Bitcoin
  transaction may already be confirmed, so its proof and ordinary timeout
  paths remain available. After a timeout returns the position to `Active`,
  it can be marked `Stranded`; a late proof of that timed-out generation
  atomically reconstructs the released anchor accounting, settles the
  confirmed transaction, and re-strands any lineage that still belongs to
  an unusable wallet. A governance-insurance hook is stubbed for a future
  compensation module.
...
* `ReservationVault` ownership transferred to governance at deploy, with
  renewals paused until governance activates them; renewal and whole-
  redemption calls carry a user fee bound (`maxFeeTbtc`). Fee schedule stays
  40 bps initiation / 20 bps/yr renewal / 20 bps redemption. A governance-
  delay posture for `updateFees` and an initiation-fee snapshot remain open
