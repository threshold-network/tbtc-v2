...
- Deliver Wormhole VAA to `SolanaDepositor` contract once the deposit was
  revealed on Solana.
- Observe `TokensTransferredWithPayload` emitted by the L1 depositor after it
  submits `transferTokensWithPayload` to the Wormhole Token Bridge. The relayer
  uses the transfer sequence from that event to fetch the signed Wormhole VAA
  and complete delivery on the destination chain.
- Submit the fetched Wormhole VAA to the destination `WormholeGateway` so the
  gateway can mint canonical tBTC for the depositor address carried in the
  payload. This replaces the older flow that relied on separate Wormhole
  Standard Relaying steps after the L1 transfer.
...
9. Relayer bot notifies `SolanaDepositor` contract the deposit was
   optimistically minted. `SolanaDepositor` inspects
   `TBTCVault.optimisticMintingRequests` to confirm the amount and submits
   `transferTokensWithPayload` to the Wormhole Token Bridge using the
   destination gateway as the recipient and the depositor address as payload.
   The same happens when the deposit was not optimistically minted but swept.
10. `SolanaDepositor` emits `TokensTransferredWithPayload`. The relayer bot
    uses the emitted transfer sequence to fetch the signed Wormhole VAA and
    submit it on Solana.
11. `SolanaGateway` verifies the VAA, mints canonical tBTC for the depositor
    address carried in the payload, and completes the transfer on Solana.
12. The user receives their tBTC under the Solana address.
