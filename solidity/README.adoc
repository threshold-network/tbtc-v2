...
`L2BTCRedeemerWormhole.initialize` expects the canonical L1 BTC redeemer
Wormhole chain ID as its fourth argument. This guard is critical for security:
without it, a misconfigured L2 could route bridged tBTC to a chain where an
attacker controls the redeemer contract. The guard pins redemptions to the
configured L1 redeemer chain.
...
- *Existing proxy upgrade:* Upon upgrading the implementation, run the
  owner-only `initializeV2(uint16)` reinitializer atomically with the
  implementation upgrade. The new `l1BtcRedeemerWormholeChain` storage slot
  reads 0 in an upgraded proxy, and redemptions fail closed until it is set.
