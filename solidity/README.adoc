...
Wormhole chain ID as its fourth argument:
...
Deployment scripts and upgrade tooling must pass the canonical Wormhole chain
ID for the configured L1 BTC redeemer. The initializer rejects a zero chain ID,
and `requestRedemption` rejects any recipient chain different from the
initialized value.
