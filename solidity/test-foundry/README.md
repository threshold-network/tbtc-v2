...
source; inspect the selected profile when adding tests for those contracts. The
`WalletRegistry` override applies only when `contracts/hardhat-dependency-compiler/`
exists locally (generated by a prior Hardhat compile; see `foundry.toml`'s
remappings comment) - the CI `contracts-foundry` job never runs a Hardhat compile
first, so that directory is absent there and the override is inert in CI.
