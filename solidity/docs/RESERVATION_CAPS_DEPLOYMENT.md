...
- **Wallet-closing gate dependency:** The safety story for `maxActiveReservations` assumes that as wallets retire, capacity is safely bounded without stranding. The wallet-closing precondition (`walletReservationInfo[wallet].count == 0`) IS enforced in `Wallets.sol` on this branch (both `beginWalletClosing` and `finalizeWalletClosing` require it), so a wallet with live reservation anchors cannot begin or finish closing without those reservations first being re-anchored, cleared, or stranded via `notifyReservationStranded` after real termination.
