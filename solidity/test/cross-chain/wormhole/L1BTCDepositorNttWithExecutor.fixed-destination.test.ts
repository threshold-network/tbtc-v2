...
describe("L1BTCDepositorNttWithExecutor fixed destination", () => {
  ...
  let depositorHarness: TestL1BTCDepositorNttWithExecutor
  ...
  before(async () => {
    ...
    const TestL1BTCDepositorNttWithExecutor = await ethers.getContractFactory(
      "TestL1BTCDepositorNttWithExecutor"
    )
    const implementation = await TestL1BTCDepositorNttWithExecutor.deploy()
    ...
    depositorHarness = TestL1BTCDepositorNttWithExecutor.attach(
      proxy.address
  ...
  it("decodes unmarked legacy packed recipients with executor", async () => {
    ...
    await depositorHarness.setFixedDestinationDepositForTest(
      fixture.expectedDepositKey,
      false
    )
    await bridge.sweepDeposit(fixture.expectedDepositKey)
