...
;(hasParityEnv ? describe : describe.skip)(
  ...
  () => {
    ...
    before(() => {
      baseline = readSnapshot(process.env.PARITY_BASELINE!)
      candidate = readSnapshot(process.env.PARITY_CANDIDATE!)
    })
    ...
    test("accepts captured compatibility while preserving the raw byte failure", () => {
      const before = [...candidate].map(([name, data]) => [
        name,
        data.toString(),
      ])
      ...
      assert.deepStrictEqual(
        [
          ...
          "deployments/",
          "export/deploy/",
        ].map((group) => [raw[group].identical, raw[group].different.length]),
        [
          [1, 0],
          [82, 3],
          [118, 27],
