...
///         - All amounts are expressed in TBTC base units (1e18).
interface IMintBurnGuard {
    ...
    function totalMintedTbtc() external view returns (uint256);

    /// @notice Returns the current global mint cap.
    /// @dev A value of zero means the global cap is not enforced.
    function globalMintCapTbtc() external view returns (uint256);
