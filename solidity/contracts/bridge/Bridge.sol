    ...
    IReceiveBalanceApproval
{
    ...
    /// @notice ABI-back-compat stub. D-2.2 slice 3 removed the
    ///         scheme setter so this event never fires post-slice-3;
    ///         the declaration is kept so out-of-tree tooling
    ///         registered for the topic does not see a missing entry.
    event NewWalletSchemeSet(BridgeState.WalletScheme indexed scheme);

    /// @notice ABI-back-compat stub. D-2.2 dropped the `emit EcdsaRetired()`
    ///         call so this event never fires; the declaration is kept
    ///         so the D-2 hard-retirement event appears in the Bridge
    ///         contract ABI for historical indexing. Off-chain consumers
    ///         should query `Bridge.ecdsaRetired()` instead.
    event EcdsaRetired();
    ...
    /// @notice Returns the address of the configured FROST wallet
    ///         registry, or address(0) until governance calls
    ///         `setFrostWalletRegistry`.
    /// @dev Read directly from BridgeState so operators can verify
    ///      the FROST activation wiring without falling back to
    ///      log-scanning the `FrostWalletRegistrySet` event.
    function frostWalletRegistry() external view returns (address) {
        return self.frostWalletRegistry;
    }

    /// @notice Returns the address of the configured BridgeLifecycleRouter,
    ///         or address(0) until governance calls `setLifecycleRouter`.
    /// @dev Read directly from BridgeState so operators can verify
    ///      the FROST activation wiring without falling back to
    ///      log-scanning the `LifecycleRouterSet` event.
    function lifecycleRouter() external view returns (address) {
        return self.lifecycleRouter;
    }
