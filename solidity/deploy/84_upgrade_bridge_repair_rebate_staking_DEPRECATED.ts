...
const func: DeployFunction =
  async function upgradeBridgeRepairRebateStakingDeprecated(
    ...
    const bridgeLibraries = {
      Deposit: await resolveAddress(
        "Deposit",
        bridgeArtifact.libraries?.Deposit
      ),
      ...
      Wallets: await resolveAddress(
        "Wallets",
        bridgeArtifact.libraries?.Wallets
      ),
