...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  // Deploy the Deposit library. hardhat-deploy detects bytecode changes
  // and deploys a new version if the compiled artifact differs from the
  // on-chain code at the address recorded in Deposit.json.
  const Deposit = await deployments.deploy("Deposit", {
    from: deployer,
    log: true,
    waitConfirmations: 1,
  })
  if (Deposit.newlyDeployed) {
    console.log(`New Deposit library deployed at: ${Deposit.address}`)
  }
