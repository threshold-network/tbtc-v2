...
3. Call `Gateway::retire_gateway_for_ntt` to consume `GatewayCapabilities` and
   return `TreasuryCap<TBTC>` to the admin.
4. Pass that `TreasuryCap<TBTC>` to Wormhole NTT's `setup::complete_burning<TBTC>`
   when initializing the Sui NTT manager.

`retire_gateway_for_ntt` enforces the pause, zero `minted_amount`, and empty
wrapped-token treasury requirements. This keeps the old gateway from minting
after the capability handoff and avoids silently converting live lockbox debt
into NTT supply.
