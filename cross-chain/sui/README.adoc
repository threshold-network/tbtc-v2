...
1. Drain or otherwise reconcile the wrapped-token treasury so the gateway has no
   outstanding lockbox-backed minted amount and holds no wrapped-token balance,
   and burn any outstanding `TreasuryCap<TBTC>` supply. This step MUST happen
   before pausing: `Gateway::send_tokens`, `Gateway::send_tokens_standard`, and
   `TBTC::burn` all require the gateway and the TBTC token to be unpaused to
   execute, so `minted_amount` and `TreasuryCap<TBTC>` total supply cannot be
   driven to zero once either is paused.
2. Pause the existing Sui Wormhole gateway (`Gateway::pause`).
3. Pause the TBTC token itself (`TBTC::pause`, via a `TBTC::GuardianCap`).
...
4. Call `Gateway::retire_gateway_for_ntt` to consume `GatewayCapabilities` and
   ...
   `TBTC::GuardianCap` belonging to a currently-registered guardian distinct
   from the admin. This is not full Solana-style dual-signing: Sui's
   owned-object transaction model requires both capabilities to be owned by
   the same transaction sender, so the check only guards against reusing the
   admin's own address as its guardian, not independent authorization from
