...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  // Deploy the new best-effort-reimbursement implementation. The staged
  // L1BTCDepositorWormholeV2Arbitrum artifact carries the best-effort
  // `finalizeDeposit` change, so `prepareUpgrade` deploys fresh bytecode that
  // differs from the current on-chain implementation.
  ...
  // also controls Base) to execute. This script does NOT broadcast the upgrade.
...
func.tags = ["UpgradeArbitrumL1BitcoinDepositorTo968"]

// Implementation-swap tooling for the live mainnet Arbitrum proxy. Guarded on
...
// `yarn deploy --tags UpgradeArbitrumL1BitcoinDepositorTo968 --network mainnet`.
