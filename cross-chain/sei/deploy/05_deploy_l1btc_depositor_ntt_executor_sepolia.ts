...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  if (hre.network.name !== 'sepolia') {
    console.log(`⏭️  Skipping Sepolia L1BTCDepositorNttWithExecutor deployment on ${hre.network.name}`)
    return
  }

  console.log('🎯 Deploying L1BTCDepositorNttWithExecutor on Sepolia...')
  
  const result = await deployL1BTCDepositorNttWithExecutor(hre, NETWORK_CONFIGS.sepolia)
...
func.tags = ["SepoliaL1BTCDepositorNttExecutor", "L1BTCDepositorNttWithExecutor"]
func.dependencies = []
