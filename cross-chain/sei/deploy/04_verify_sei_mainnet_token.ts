...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  if (hre.network.name !== 'pacific-1') {
    console.log(`⏭️  Skipping Sei Pacific-1 verification on ${hre.network.name}`)
    return
  }
  ...
  if (!proxyAddress) {
    throw new Error('Please provide PROXY_ADDRESS environment variable for Sei Pacific-1 verification')
  }
