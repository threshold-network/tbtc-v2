...
async function main() {
  ...
  if (!accountNumber || isNaN(Number(accountNumber))) {
    ...
    console.log('Usage: ACCOUNT_NUMBER=2 npx hardhat run scripts/encrypt-account.ts');
    console.log('Example: ACCOUNT_NUMBER=2 npx hardhat run scripts/encrypt-account.ts');
    process.exit(1);
  }
  ...
  console.log(`🔐 Setting up encrypted Account #${accountNumber} key storage...`);
  ...
  console.log(`✅ Account #${accountNumber} private key encrypted and stored securely`);
  ...
  console.log(`   2. Run: ACCOUNT_NUMBER=${accountNumber} npx hardhat run scripts/use-account.ts --network baseSepolia`);
}
