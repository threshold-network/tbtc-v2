...
export async function verifyDeployment(
  ...
  const networkConfigs: Record<
    string,
    { rpcUrl: string; explorer: string; name: string }
  > = {
    sei_atlantic_2: {
  ...
  if (!networkConfig) {
    throw new Error(
      `Unsupported network: ${networkName}. Supported: ${Object.keys(
        networkConfigs
      ).join(", ")}`
    )
  }
  ...
  console.log(
    `💰 Deployer balance: ${ethers.utils.formatEther(ethBalance)} ETH`
  )
  ...
  try {
    ...
    try {
      ...
      console.log(
        `   All Minters: ${minters.length === 0 ? "None" : minters.join(", ")}`
      )
    } catch (error) {
    ...
    if (owner.toLowerCase() === deployer.toLowerCase()) {
      ...
      if (!isMinter) {
        ...
        if (isMinterAfterAdd) {
          ...
          console.log(
            `   Final Total Supply: ${ethers.utils.formatEther(
              finalTotalSupply
            )} ${symbol}`
          )
        } else {
          console.log(
            "   ⚠️  Warning: Minter addition may not have taken effect properly"
          )
        }
      } else {
    ...
    console.log(
      `🔗 View on explorer: ${networkConfig.explorer}/address/${proxyAddress}`
    )
...
async function main() {
  ...
  const proxyAddress =
    process.env.PROXY_ADDRESS || "0x7fE80D5A582393715C5f6381F42E8Ce6Ff584ef4"
