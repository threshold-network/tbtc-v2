...
async function getDecryptedAccountKey(accountNumber: string): Promise<string> {
  const encryptedKeyFile = accountNumber === '1' ? '.encrypted-key' : `.encrypted-key-${accountNumber}`;
  const encryptedKeyPath = path.join(__dirname, '..', encryptedKeyFile);

  if (!fs.existsSync(encryptedKeyPath)) {
    throw new Error(`No encrypted Account #${accountNumber} key found. Run encrypt-account.ts first.`);
  }
...
async function main() {
  ...
  if (!accountNumber || isNaN(Number(accountNumber))) {
    ...
    console.log('Usage: ACCOUNT_NUMBER=2 npx hardhat run scripts/use-account.ts --network <network>');
    console.log('Example: ACCOUNT_NUMBER=2 npx hardhat run scripts/use-account.ts --network baseSepolia');
    process.exit(1);
  }

  console.log(`🔧 Using Account #${accountNumber} to interact with L2TBTC contract...`);
  ...
  try {
    ...
    console.log('   Current tBTC Balance:', ethers.utils.formatEther(tbtcBalance), 'tBTC');

    // Transfer ownership if proxy is still the owner or if owner is zero address
    if ((owner === proxyAddress || owner === '0x0000000000000000000000000000000000000000') && wallet.address !== owner) {
    ...
    console.log(`   Account #${accountNumber} tBTC Balance:`, ethers.utils.formatEther(finalTbtcBalance), 'tBTC');
    ...
    console.log('🔗 Explorer: https://sepolia.basescan.org/address/' + proxyAddress);
