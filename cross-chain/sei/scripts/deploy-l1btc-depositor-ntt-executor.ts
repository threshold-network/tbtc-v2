...
export async function deployL1BTCDepositorNttWithExecutor(
  ...
  const setSupportedChainTx = await proxy.setSupportedChain(networkConfig.seiChainId, true);
  ...
  const setDefaultChainTx = await proxy.setDefaultSupportedChain(networkConfig.seiChainId);
  await setDefaultChainTx.wait();

  // If Base Sepolia is configured (for testing), add it as well
  if (networkConfig.baseSepolia) {
    console.log(`   Setting supported chain: ${networkConfig.baseSepolia.chainId} (Base Sepolia)`);
    const setBaseSupportedChainTx = await proxy.setSupportedChain(networkConfig.baseSepolia.chainId, true);
  ...
  console.log(`   Implementation: ${implementationAddress}`);
  console.log(`   Constructor Args: [${networkConfig.tbtcBridge}, ${networkConfig.tbtcVault}, ${networkConfig.nttManagerWithExecutor}, ${networkConfig.underlyingNttManager}]`);
...
async function main() {
  ...
  const result = await deployL1BTCDepositorNttWithExecutor(hre, NETWORK_CONFIGS[network]);
