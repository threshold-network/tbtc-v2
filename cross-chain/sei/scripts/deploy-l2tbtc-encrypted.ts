...
export async function deployL2TBTC(
  ...
  try {
    ...
    console.log(
      "   💡 Use verify-deployment.ts script for detailed verification"
    )
  }

  // Contract verification for block explorers
  if (
    hre.network.tags?.basescan ||
    hre.network.tags?.seitrace ||
...
async function main() {
  ...
  console.log(
    "   npx hardhat deploy --network sei_atlantic_2 --tags SeiTestnet"
  )
  console.log("   npx hardhat deploy --network baseSepolia --tags BaseSepolia")
  console.log(
    "   npx hardhat deploy --network ethereumSepolia --tags EthereumSepolia"
  )
